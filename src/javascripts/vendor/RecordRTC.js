...
function RecordRTC(mediaStream, config) {
  ...
  function initRecorder(initCallback) {
    ...
    let Recorder = new GetRecorderType(mediaStream, config);
  ...
  function getDataURL(callback, _mediaRecorder) {
    ...
    if (typeof Worker !== 'undefined' && !navigator.mozGetUserMedia) {
      let webWorker = processInWebWorker(readFile);
  ...
  let WARNING = `It seems that recorder is destroyed or "startRecording" is not invoked for ${config.type} recorder.`;
...
RecordRTC.getFromDisk = function(type, callback) {
  ...
  console.log(
    `Getting recorded ${type === 'all' ? 'blobs' : type + ' blob '} from disk!`,
  );
  ...
  this.startRecording = function() {
    ...
    if (mediaType.video) {
      ...
      if (
        ...
        let videoTrack = getTracks(mediaStream, 'video')[0];
  ...
  this.getDataURL = function(callback) {
    ...
    function getDataURL(blob, callback00) {
      if (typeof Worker !== 'undefined') {
        let webWorker = processInWebWorker(_blob => {
          postMessage(new FileReaderSync().readAsDataURL(_blob));
        });
...
function getTracks(stream, kind) {
  ...
  return stream.getTracks().filter(t => {
...
function StereoAudioRecorder(mediaStream, config) {
  ...
  if (!config.disableLogs) {
    console.log(
      `StereoAudioRecorder is set to record number of channels: ${numberOfAudioChannels}`,
    );
  }
  ...
  function mergeLeftRightBuffers(config, callback) {
    ...
    let webWorker = processInWebWorker(mergeAudioBuffers);
  ...
  let context = Storage.AudioContextConstructor;

  // creates an audio node from the microphone incoming stream
  let audioInput = context.createMediaStreamSource(mediaStream);

  let legalBufferValues = [0, 256, 512, 1024, 2048, 4096, 8192, 16384];
  ...
  if (legalBufferValues.indexOf(bufferSize) === -1) {
    if (!config.disableLogs) {
      console.log(
        `Legal values for buffer-size are ${JSON.stringify(
          legalBufferValues,
          null,
          '\t',
        )}`,
  ...
  function onAudioProcessDataAvailable(e) {
    ...
    let chLeft = new Float32Array(left);
...
function CanvasRecorder(htmlElement, config) {
  ...
  let matchArray = navigator.userAgent.match(/Chrom(e|ium)\/([0-9]+)\./);
  ...
  this.getWebPImages = function(callback) {
    ...
    let framesLength = whammy.frames.length;
